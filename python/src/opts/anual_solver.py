...
class AnualSolver:
    ...
    def define_variables(self) -> None:
            ...
            for d in self.dto.day_of_week:
                for p in self.dto.schedule[h][d]:
                    for block in self.dto.curriculum_dict[h]:
                        for lane in block:
                            for c in lane:
                                self.x[h, d, p, c] = pulp.LpVariable(
                                    name=f"x_{h}_{d}_{p}_{c}", cat="Binary")
        # yの定義
        for d in self.dto.day_of_week:
            for p in range(1, 8):
                for t in self.dto.teacher_list:
                    self.y[d, p, t] = pulp.LpVariable(
                        name=f"y_{d}_{p}_{t}", cat="Binary")
        ...
        for t in self.dto.teacher_list:
            ...
            for d in self.dto.day_of_week:
                for p in range(1, 8):
                    self.y[d, p, t] = pulp.lpSum([self.x[h, d, p, c] for (
                        h, c) in first_hcs if (h, d, p, c) in self.x])
